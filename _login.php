...
$res2 = mysqli_query($connection, "SELECT * FROM `ss_users` WHERE `username` = '$user_email' AND `password` = '$user_pass'");

if (mysqli_num_rows($res) > 0 || mysqli_num_rows($res2) > 0) {
	$final;
	if(mysqli_num_rows($res) > 0) $final = $res;
	if(mysqli_num_rows($res2) > 0) $final = $res2;
